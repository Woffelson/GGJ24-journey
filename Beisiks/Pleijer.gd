...
func _physics_process(delta):
	...
	hor = Input.get_axis("key_left", "key_right")
	if hor != 0:
	...
	if Input.is_action_just_pressed("key_left"): torso.scale.x = 2 #hnngghh toimi ny
	elif Input.is_action_just_pressed("key_right"): torso.scale.x = -2
